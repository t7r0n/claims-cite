...
COMPANY = 'Solva'
REPO = 'claims-cite'
PROJECT_TERMS = ['solva', 'wedge', 'recommendation', 'source', 'cited', 'claim', 'policy', 'documents']
PROJECT_METRICS = ['solva_coverage', 'wedge_risk', 'recommendation_precision', 'source_latency']
PROJECT_FAILURES = ['solva_drift', 'wedge_gap', 'recommendation_misroute', 'source_blindspot']
PROJECT_ARCHETYPES = [{'name': 'solva evidence replay', 'trigger': 'solva signal changes while wedge context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'wedge boundary probe', 'trigger': 'wedge handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'recommendation regression harness', 'trigger': 'recommendation behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'source operator packet', 'trigger': 'source output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A reproducible benchmark + verification toolkit that scores any claims handling AI on citation faithfulness under adversarial document perturbations, designed to be the artifact a carrier procurement team can run before signing Solva.'
...
HERO_TITLE = 'Solva Adversarial Replay Board'
